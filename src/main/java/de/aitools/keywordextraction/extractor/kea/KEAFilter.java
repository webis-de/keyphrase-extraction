...
import de.aitools.keywordextraction.extractor.kea.weka.classifiers.DistributionClassifier;
import de.aitools.keywordextraction.extractor.kea.weka.classifiers.FilteredClassifier;
...
import de.aitools.keywordextraction.extractor.kea.weka.filters.DiscretizeFilter;
...
public class KEAFilter extends Filter implements OptionHandler {
  ...
  private DistributionClassifier m_Classifier = null;
  ...
  private void buildClassifier() throws Exception {
    ...
    fclass.setClassifier(new de.aitools.keywordextraction.extractor.kea.weka.classifiers.NaiveBayesSimple());
    fclass.setFilter(new DiscretizeFilter());
    m_Classifier = fclass;
  ...
  private void convertPendingInstances() throws Exception {
    ...
    for (int i = 0; i < getInputFormat().numAttributes(); i++) {
      if (i == m_DocumentAtt) {
	atts.addElement(new Attribute("N-gram", null));
	atts.addElement(new Attribute("N-gram-original", null));
	atts.addElement(new Attribute("TFxIDF"));
	atts.addElement(new Attribute("First_occurrence"));
	if (m_Debug) {
	  if (m_KFused) {
